...
// ÞARF AÐ LAGA: VAKT 23-8 16. JÚNÍ ER EKKI TALIN SEM STÓRHÁTÍÐ!!!

const weekdays = ['sunnudagur', 'mánudagur', 'þriðjudagur', 'miðvikudagur', 'fimmtudagur', 'föstudagur', 'laugardagur'];
const storhatid = ["1-1", "4-15", "4-17", "5-23", "6-17", "8-2", "12-25"];
const helgidagar = ["4-14", "4-18", "4-21", "5-1", "5-13", "5-24", "12-26"];
...
function getWageBasis(bracket, step, rt, year) {
    if (year == 2021) {
        return Math.round(monthly2021[bracket]*(1+step*0.015)*(rt/100));
    } else if (year == 2022) {
        return Math.round(monthly2022[bracket]*(1+step*0.015)*(rt/100));
    }
}
...
function processNewDays() {
    ...
    if (mode == "average") {
        ...
        while (dt.getMonth() == month) {
            ...
            var w = Math.floor((startingPoint + day)/7) % weekCount;
            var d = (startingPoint + day) % 7;
            ...
            if (captionEl.innerHTML.length > weekdays[(firstDay + d) % 7].length)
                captionEl.innerHTML += " ofl";
            else
                captionEl.innerHTML = weekdays[(firstDay + d) % 7] + " " + dt.getDate().toString() + "/" + (dt.getMonth()+1).toString();
    ...
    if (oldSetup) {
        ...
        if (mode == "average") {
            ...
            while (dt.getMonth() == month) {
                ...
                w = Math.floor((startingPoint + day)/7) % weekCount;
                d = (startingPoint + day) % 7;
...
function getShiftBonus(hrs, wageBracket, wageStep, shiftCount, ratio) {
    var basis = monthly2021[wageBracket] * (1 + wageStep * 0.015) * ratio / 100;
    var types = -1;
...
function getPremiums(t1, t2, theDate, checkHolidays=false) {
    ...
    if (dayID == "12-24" && checkHolidays) {
        ...
        p_120 = evening;
        p_90 = day + night;
    } else if (storhatid.indexOf(dayID) !== -1 && checkHolidays) { // stórhátíð
        p_90 = night + day + evening;
    } else if ((theDate.getDay() + 1) % 7 < 2 || (checkHolidays && helgidagar.indexOf(dayID) !== -1)) { // helgidagar
        p_75 = night;
        p_55 = day + evening;
...
function displayOld() {
    ...
    document.getElementById('old-caption').innerHTML = "Gamla rúllan";
}
...
function comparison() {
    ...
    if (current == "average") {
        ...
        document.getElementById('diff-month-label').innerHTML = "Á mánuði";
    } else {
        ...
        document.getElementById('diff-month-label').innerHTML = "Í " + selector.options[selector.selectedIndex].text;
    }
